...
impl Grid {
    ...
    pub fn new(
        ...
        let matrix = {
            ...
            Array2::from_shape_simple_fn(Dim([cols, rows]), || {
    ...
    pub fn get_near_cubes(
        ...
        let x_max = self.matrix.len_of(Axis(0)) as i32 - 1;
        let y_max = self.matrix.len_of(Axis(1)) as i32 - 1;
        match quantity {
...
fn three_by_three(
    ...
    relative_indexes
        .map(|(x_rel, y_rel)| {
            ...
            let color = compute_color(grid_ref.get_minmax(), temperature)
                .color
                .to_srgba();
            let red = color.red;
            let blue = color.blue;
